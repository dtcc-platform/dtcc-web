...
const wizardEditHref = computed(() =>
  slug && isValidSlug(slug) ? withBase(`post/?section=events-archive&slug=${encodeURIComponent(slug)}`) : ''
)
...
onMounted(async () => {
  ...
  try {
    ...
    related.value = await fetchRelatedItems(relatedSlugs, 'content/dtcc-1', 3)
